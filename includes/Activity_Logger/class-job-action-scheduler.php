...
final class Job_Action_Scheduler {
	...
	/**
	 * Prevents nested chain_next_slice_immediately calls in one request.
	 *
	 * @var bool
	 */
	private static $chaining_next = false;
	...
	public static function init() {
		...
		// Disable AS HTTP async during bulk — concurrent loopbacks + AS async
		// stampeded PHP-FPM and produced 503 / "اتصال به سرور کند است".
	...
	public static function enqueue_next( $force = false, $delay_sec = null ) {
		...
		\PolymartAI\Activity_Logger::on_as_chain_enqueued( absint( $action_id ) );
	...
	public static function ensure_scheduled() {
		...
		if ( ! \PolymartAI\Activity_Logger::is_bulk_worker_lively( self::STALE_RUNNING_SEC ) ) {
			$recovered = self::recover_stale_running_actions( self::STALE_RUNNING_SEC );
			\PolymartAI\Activity_Logger::release_stale_locks_for_as( self::STALE_RUNNING_SEC );
		}
		...
		if ( \PolymartAI\Activity_Logger::should_prioritize_elementor_partial( $job ) ) {
			if ( ! \PolymartAI\Activity_Logger::is_bulk_worker_lively( self::STALE_RUNNING_SEC ) ) {
				self::recover_stale_running_actions( self::STALE_RUNNING_SEC );
				\PolymartAI\Activity_Logger::release_stale_locks_for_as( self::STALE_RUNNING_SEC );
			...
			if (
				self::count_actions_by_status( \ActionScheduler_Store::STATUS_RUNNING ) > 0
				&& \PolymartAI\Activity_Logger::is_bulk_worker_lively( 75 )
			) {
				return 0;
			}
		...
		if ( self::count_actions_by_status( \ActionScheduler_Store::STATUS_PENDING ) > 0 ) {
			if ( ! \PolymartAI\Activity_Logger::is_bulk_worker_lively( 75 ) ) {
	...
	private static function run_slice_action_batch() {
		...
		if ( $elementor_burst ) {
			...
			while (
				...
				try {
					$step_result = \PolymartAI\Activity_Logger::run_action_scheduler_batch(
				...
				if ( ! \PolymartAI\Activity_Logger::should_prioritize_elementor_partial( $job_step ) ) {
					break;
				}
			}
		} else {
			while (
				...
				try {
					$step_result = \PolymartAI\Activity_Logger::run_action_scheduler_batch( max( 1, $step_cap ), $remaining_budget );
				} catch ( \Throwable $e ) {
					\PolymartAI\Activity_Logger::log(
						'error',
						sprintf(
				...
				$steps_after = absint( \PolymartAI\Activity_Logger::get_job_for_as_debug()['steps'] ?? 0 );
				$idle_tick   = is_array( $step_result ) && ! empty( $step_result['idle_tick'] );
				...
				if ( 'running' !== (string) ( $step_result['status'] ?? \PolymartAI\Activity_Logger::get_job_for_as_debug()['status'] ?? '' ) ) {
					break;
				}
			}
		}

		$job_after  = \PolymartAI\Activity_Logger::get_job_for_as_debug();
	...
	public static function handler_shutdown_recovery() {
		...
		if ( self::$handler_clean_exit ) {
			if (
				...
				if ( ! self::has_pending_or_running() ) {
					...
					// Soft cron wake only — loopback here flooded PHP-FPM (503).
					\PolymartAI\Activity_Logger::nudge_as_queue_runner();
				}
			}
		...
		try {
			...
			if ( $post_id > 0 ) {
				$lang = sanitize_key( (string) ( \PolymartAI\Activity_Logger::get_job_for_as_debug()['lang'] ?? 'en' ) );
			...
			\PolymartAI\Activity_Logger::recover_worker_infrastructure_failure(
				$message,
				array(
					'source'       => 'as_shutdown',
	...
	private static function chain_next_slice_immediately() {
		...
		try {
			...
			// Soft-wake via wp-cron pulse (not HTTP loopback — that caused 503).
	...
	private static function try_claim_slice_mutex( $action_id ) {
		...
		if ( false !== $held ) {
			...
			if ( $held_id > 0 && $held_id !== $action_id && \PolymartAI\Activity_Logger::is_bulk_worker_lively( self::SLICE_BUDGET_SEC ) ) {
				return false;
			}
		}
	...
	private static function defer_slice_action( $action_id ) {
		...
		if ( function_exists( 'as_schedule_single_action' ) && \PolymartAI\Activity_Logger::is_bulk_job_running() ) {
	...
	public static function clear_slice_mutex_if_stale() {
		if ( ! \PolymartAI\Activity_Logger::is_bulk_worker_lively( self::SLICE_BUDGET_SEC ) ) {
			self::clear_slice_mutex();
		}
	}
	...
	public static function run_queue_inline( $force_inline = false ) {
		...
		$job        = \PolymartAI\Activity_Logger::get_job_for_as_debug();
		$elementor  = \PolymartAI\Activity_Logger::should_prioritize_elementor_partial( $job );
		...
		if ( $done <= 0 && ( $force_inline || $claim_failed ) && \PolymartAI\Activity_Logger::is_bulk_job_running() ) {
			...
			if ( \PolymartAI\Activity_Logger::should_prioritize_elementor_partial( $job ) ) {
				$cap = \PolymartAI\Activity_Logger::get_elementor_partial_step_cap( $job );
				\PolymartAI\Activity_Logger::run_action_scheduler_batch( max( 1, $cap ), self::SLICE_BUDGET_SEC );
			} else {
				\PolymartAI\Activity_Logger::run_action_scheduler_batch( 1, self::SLICE_BUDGET_SEC );
			}
